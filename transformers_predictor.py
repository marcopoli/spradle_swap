...
if __name__ == '__main__':
    model_path = "model_files/bert_multi"
    ...
    for filename in os.listdir('data/spradle/test_heldOut/'):
        if filename != ".DS_Store" and filename.split(".")[-1] == 'txt':
            print(filename)
            f = open('data/spradle/test_heldOut/' + filename, 'r')
            text = ""
            i = 0
            len0 = 0
            for line in f:
            ...
            f = open('data/spradle/res_loc/' + filename.split('.')[0] + ".ann", "w")
            pos = 0
            j = 0
            for ann, tok in zip(res[0], text):
                in_pos = pos
                tok = tok.replace("\n","")
                fin_pos = pos + len(tok)
                pos = fin_pos +1
                f.write("T" + str(j) + "\t" + ann + "\t" + str(in_pos) + "\t" + str(fin_pos) + "\t" + tok + '\n')
                j = j+1
